...
<?php 
	...
	if(!$ares_id) //Create new campaign with template
		$q = 'INSERT INTO campaigns (userID, app, from_name, from_email, reply_to, query_string, title, html_text, plain_text, wysiwyg) VALUES ('.get_app_info('main_userID').', '.$aid.', "'.$from_name.'", "'.$from_email.'", "'.$reply_to.'", "'.$query_string.'", "'.$template_name.'", "'.addslashes($html_text).'", "'.addslashes($plain_text).'", 1)';
	else
		$q = 'INSERT INTO ares_emails (ares_id, from_name, from_email, reply_to, query_string, title, html_text, plain_text, wysiwyg, created, time_condition '.$segmentation_line1.') VALUES ('.$ares_id.', "'.$from_name.'", "'.$from_email.'", "'.$reply_to.'", "'.$query_string.'", "'.$template_name.'", "'.addslashes($html_text).'", "'.addslashes($plain_text).'", 1, '.time().', "'.$time_condition.'" '.$segmentation_line2.')';
	$r = mysqli_query($mysqli, $q);
	if ($r)
	{
