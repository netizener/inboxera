...
<?php 
	...
	$save_as_template = $_POST['save_as_template']=='on' ? true : false;
	...
	else
	{
		...
		$q = 'INSERT INTO campaigns (userID, app, from_name, from_email, reply_to, title, label, plain_text, html_text, json_design, query_string, wysiwyg, bounce_setup, complaint_setup, opens_tracking, links_tracking) VALUES ('.get_app_info('main_userID').', '.get_app_info('app').', "'.$from_name.'", "'.$from_email.'", "'.$reply_to.'", "'.$subject.'", "'.$campaign_title.'", "'.$plain.'", "'.addslashes($html).'", "'.addslashes($jsondesign).'", "'.$query_string.'", '.$wysiwyg.', '.$bounce_setup.', '.$complaint_setup.', '.$track_opens.', '.$track_clicks.')';
		$r = mysqli_query($mysqli, $q);
		if ($r)
		{
